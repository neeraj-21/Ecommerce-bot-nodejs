...
const apiVersion = 'v3.3';//'v2.8';

class FBeamer {
  constructor({ pageAccessToken, verifyToken, appSecret, SERVER_URL }) {
    try {
      if (pageAccessToken && verifyToken && appSecret && SERVER_URL) {
  ...
  webview(id, text, messaging_type = 'RESPONSE') {
    let obj = {
      recipient: {
        id
      },
